...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        [object[]]$params = (Get-ChildItem Function:\New-DcnClone).Parameters.Keys
        $knownParameters = 'SqlInstance', 'SqlCredential', 'DcnSqlCredential', 'Credential', 'ParentVhd', 'Destination', 'CloneName', 'Database', 'LatestImage', 'Disabled', 'Force', 'EnableException'
...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    BeforeAll {
        ...
        $null = Set-DcnConfiguration -InformationStore File -Path $script:clonefolder -Force
    }
