...
$text -join "`n`n" | Set-Content -Path "$($publishDir.FullName)\PSDatabaseClone\commands.ps1"
...
if ($text) { $text -join "`n`n" | Set-Content -Path "$($publishDir.FullName)\PSDatabaseClone\resourcesBefore.ps1" }
